...
If `d` denotes a matrix of non-spatial distances between data objects, and `x`
...
provide more a means of attaining approximately spatially-structured clustering
...
(whether between points or finite objects). Conventional approaches combine
these two matrices into one prior to clustering; spatially-constrained
clustering is performed on the matrix of distances between (non-spatial) data
objects, while the formation of clusters is constrained such that only
neighbouring points or objects are merged into the same cluster.
...
[@Guo2008] constructs minimal spanning trees through a set of spatial neighbours,
and then
